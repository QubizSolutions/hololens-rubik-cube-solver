...
namespace TwoPhase
{
    ...
    public class CoordCube
    {
        ...
        private static short[][] s_twistMove = new short[N_TWIST][];
        private static short[][] s_flipMove = new short[N_FLIP][];
        private static short[][] s_FRtoBR_Move = new short[N_FRtoBR][];
        private static short[][] s_URFtoDLF_Move = new short[N_URFtoDLF][];
        private static short[][] s_URtoDF_Move = new short[N_URtoDF][];
        private static short[][] s_URtoUL_Move = new short[N_URtoUL][];
        private static short[][] s_UBtoDF_Move = new short[N_UBtoDF][];
        private static short[][] s_MergeURtoULandUBtoDF = new short[336][];
        ...
        {
            Debug.Log("here is the prun path : " + PrunPath);
            if (File.Exists(PrunPath))
            {
                s_twistMove = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_flipMove = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_FRtoBR_Move = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_URFtoDLF_Move = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_URtoDF_Move = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_URtoUL_Move = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_UBtoDF_Move = File.ReadAllText(PrunPath).Deserialize<short[][]>();
                s_MergeURtoULandUBtoDF = File.ReadAllText(PrunPath).Deserialize<short[][]>();

                twistMove = AfterDeserializing(s_twistMove);
                flipMove = AfterDeserializing(s_flipMove);
                FRtoBR_Move = AfterDeserializing(s_FRtoBR_Move);
                URFtoDLF_Move = AfterDeserializing(s_URFtoDLF_Move);
                URtoDF_Move = AfterDeserializing(s_URtoUL_Move);
                URtoUL_Move = AfterDeserializing(s_URtoUL_Move);
                UBtoDF_Move = AfterDeserializing(s_UBtoDF_Move);
                MergeURtoULandUBtoDF = AfterDeserializing(s_MergeURtoULandUBtoDF);
                Slice_URFtoDLF_Parity_Prun = File.ReadAllText(PrunPath).Deserialize<sbyte[]>();
                Slice_URtoDF_Parity_Prun = File.ReadAllText(PrunPath).Deserialize<sbyte[]>();
                Slice_Twist_Prun = File.ReadAllText(PrunPath).Deserialize<sbyte[]>();
                Slice_Flip_Prun = File.ReadAllText(PrunPath).Deserialize<sbyte[]>();
        ...
        private static void SavePrunData()
        {
            ...
            s_twistMove = BeforeSerializing(twistMove);
            s_flipMove = BeforeSerializing(flipMove);
            s_FRtoBR_Move = BeforeSerializing(FRtoBR_Move);
            s_URFtoDLF_Move = BeforeSerializing(URFtoDLF_Move);
            s_URtoDF_Move = BeforeSerializing(URtoDF_Move);
            s_URtoUL_Move = BeforeSerializing(URtoUL_Move);
            s_UBtoDF_Move = BeforeSerializing(UBtoDF_Move);
            s_MergeURtoULandUBtoDF = BeforeSerializing(MergeURtoULandUBtoDF);

            File.WriteAllText(PrunPath, s_twistMove.Serialize());
            File.WriteAllText(PrunPath, s_flipMove.Serialize());
            File.WriteAllText(PrunPath, s_FRtoBR_Move.Serialize());
            File.WriteAllText(PrunPath, s_URFtoDLF_Move.Serialize());
            File.WriteAllText(PrunPath, s_URtoDF_Move.Serialize());
            File.WriteAllText(PrunPath, s_URtoUL_Move.Serialize());
            File.WriteAllText(PrunPath, s_UBtoDF_Move.Serialize());
            File.WriteAllText(PrunPath, s_MergeURtoULandUBtoDF.Serialize());
            File.WriteAllText(PrunPath, Slice_URFtoDLF_Parity_Prun.Serialize());
            File.WriteAllText(PrunPath, Slice_URtoDF_Parity_Prun.Serialize());
            File.WriteAllText(PrunPath, Slice_Twist_Prun.Serialize());
            File.WriteAllText(PrunPath, Slice_Flip_Prun.Serialize());
        }
        ...
        private static short[][] BeforeSerializing(short[,] data)
        {
            int dim0 = data.GetLength(0);
            int dim1 = data.GetLength(1);

            short[][] result = new short[dim0][];

            for(int i = 0; i < dim0; i++)
            {
                result[i] = new short[dim1];
        ...
        private static short[,] AfterDeserializing(short[][] data)
        {
            int dim0 = data.Length;
            int dim1 = data[0].Length;

            short[,] result = new short[dim0, dim1];
